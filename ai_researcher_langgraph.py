...
#     """Start a new conversation with unique thread_id"""
...
# === Model ===
from langchain_google_genai import ChatGoogleGenerativeAI
model = ChatGoogleGenerativeAI(model="gemini-1.5-pro").bind_tools(tools)
...
# === Exported Functions for FastAPI ===
def get_new_config():
    thread_id = str(uuid.uuid4())
    return {"configurable": {"thread_id": thread_id}}

def get_config(thread_id: str):
    """Return a config with thread_id for stateful sessions"""
    return {"configurable": {"thread_id": thread_id}}
